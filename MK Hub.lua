...
local sirenHeight = 50
local cartoonRadius = 20
local cartoonHeight = 30
...
local function moveAround(hrp, radius, height)
    ...
    if farmMode == "Orbit" then
        newPos = targetPos + Vector3.new(math.cos(t)*radius, height, math.sin(t)*radius)

    elseif farmMode == "Square" then
        local step = math.floor(t % 4)
        local pos = {
            Vector3.new(radius,height,radius),
            Vector3.new(-radius,height,radius),
            Vector3.new(-radius,height,-radius),
            Vector3.new(radius,height,-radius)
        }
        newPos = targetPos + pos[step+1]

    elseif farmMode == "Line" then
        newPos = targetPos + Vector3.new(math.sin(t)*radius,height,0)

    elseif farmMode == "Spiral" then
        local r = radius + math.sin(t)*5
        newPos = targetPos + Vector3.new(math.cos(t)*r,height,math.sin(t)*r)

    elseif farmMode == "Behind" then
        local look = hrp.CFrame.LookVector
        newPos = targetPos - (look*radius) + Vector3.new(0,height,0)

    elseif farmMode == "ZigZag" then
        newPos = targetPos + Vector3.new(math.sin(t*3)*radius,height,math.cos(t)*radius)

    elseif farmMode == "Cross" then
        local step = math.floor(t % 4)
        local pos = {
        ...
        if autoLoot then
            task.spawn(function()
                ...
                while autoLoot do
                    ...
                    if not cratesFolder then continue end
        ...
        if v then
            task.spawn(function()
                while autoSirenNPC do
                    task.wait()
                    local m = workspace:FindFirstChild("scps") and workspace.scps:FindFirstChild("real_siren")
                    if m and m:FindFirstChild("HumanoidRootPart") then
                        moveAround(m.HumanoidRootPart, sirenRadius, sirenHeight)
                    end
                end
            end)
        end
        ...
        if v then
            task.spawn(function()
                while autoSirenPlayer do
                    task.wait()
                    local m = workspace:FindFirstChild("scps") and workspace.scps:FindFirstChild("siren")
                    if m and m:FindFirstChild("HumanoidRootPart") then
                        moveAround(m.HumanoidRootPart, sirenRadius, sirenHeight)
                    end
                end
        ...
        if v then
            task.spawn(function()
                while autoCartoonNPC do
                    task.wait()
                    local m = workspace:FindFirstChild("scps") and workspace.scps:FindFirstChild("real_cartoon_cat")
                    if m and m:FindFirstChild("HumanoidRootPart") then
                        moveAround(m.HumanoidRootPart, cartoonRadius, cartoonHeight)
                    end
                end
            end)
        end
        ...
        if v then
            task.spawn(function()
                while autoCartoonPlayer do
                    task.wait()
                    local m = workspace:FindFirstChild("scps") and workspace.scps:FindFirstChild("cartoon_cat")
                    if m and m:FindFirstChild("HumanoidRootPart") then
                        moveAround(m.HumanoidRootPart, cartoonRadius, cartoonHeight)
                    end
                end
...
FarmTab:CreateDropdown({
    Name = "Farm Style",
    Options = {"Orbit","Square","Line","Spiral","Behind","ZigZag","Cross"},
    CurrentOption = {"Orbit"},
        ...
        if cratesFolder then
            for _, crate in pairs(cratesFolder:GetChildren()) do
                if crate.Name == "crate" then
                    ...
                    local label = "Crate ("..math.floor(distance)..")"
...
ESPTab:CreateToggle({Name="ESP Crates",Callback=function(v)CrateESP=v end})
